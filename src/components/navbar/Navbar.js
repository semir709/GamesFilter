...
const Navbar = ({ setNewSelected }) => {
  ...
  return (
    <header
      className={`${
        showNavBar && "navbar-holder-show "
      }  navbar-holder d-flex justify-content-between align-items-center mt-3 px-3`}
    >
      <Link to={"/"} onClick={() => setNewSelected("All Games")}>
        Logo
      </Link>
      ...
      <div className="search-wrapper">
        ...
        <div className="search-res-wrapper">
          {searchMenu &&
            data.map(({ name, id, background_image }) => {
              return (
